...
// This pseudo-UGen is a workaround for the broken AverageOutput
AverageOut {
	*ar { arg in, trig;
		var acc, avg, counter;

		counter = Phasor.ar(trig, 1, 0, 1000000, 0);
		acc = Phasor.ar(trig, in, 0, 10000, 0.0);
	...
	*compile { | libname, nHarmonics, tau, minFrequency, minSamples |
		...
		SynthDef(namePeakFollower,
			{ | aiBusConditionedEGG,
				...
				z = Dolansky.ar(dEGG, 0.99, 0.995);		// Trigger on EGG cycles
				Out.ar(aoBusGate, [z]);
			}
		).add(libname);
		...
		SynthDef(nameNDFTs,
			{ | aiBusConditionedEGG,
				...
				var in, gcycle, gDFT, gSkipped, length, gLength, inPower, invLength, amps, phases;
				var res, complex, complexAbs;
				var residualPower, harmPower;
				var inAcc;
				...
				// Compute total power in the analyzed harmonics
				harmPower = Gate.ar(complexAbs.squared.sum, gDFT);

				// accumulate the total power in the input signal; reset on "gcycle"
				// inAcc = AverageOutput.ar(in.squared, gcycle);  // AverageOutput.ar is broken
				// inPower = Gate.ar(VariadicDelay.ar(inAcc, gcycle, [gDFT + gSkipped], 600), gDFT);
				inAcc = AverageOut.ar(in.squared, gcycle); // Use our pseudo-UGen instead
				inPower = Gate.ar(VariadicDelay.ar(inAcc, gcycle, gDFT, 882), gDFT);
				residualPower = (inPower-harmPower).max(0.00001); // prevent values <= 0

				complexAbs = complexAbs.add(residualPower);  // append as last element
		...
		SynthDef(nameQciDEGG,
			{ | aiBusConditionedEGG,
				...
				var max = RunningMax.ar(sig, gc);
				var min = RunningMin.ar(sig, gc);
				peak2peak = min - max;
				...
				// Delay both of them, to synch with the DFT calculations
