	...
	*new { | view |
	...
	init { | view |
		...
		DrawGrid( Rect(),
			...
			GridLines(ControlSpec(nMinDb.asInteger, nMaxDb.asInteger, default: 0, units: "dB"))
		)
		...
		if ( mStarted.not, { // All disabled while it is started
			...
			if ( mSliderCluster.step != req_step, {
				...
				.action_{
					...
					this.changed(this, \selectCluster, mSliderCluster.value);
				};
		...
		value ({
			Dialog.openPanel({ 	| path |
				...
				try {
					...
					// Set the members
					mButtonStart.value = iPrelearned;
					mNumberBoxClusters.value = c;
					mNumberBoxHarmonics.value = h;
					mGlyphs = (h - 1) collect: { | n | (n + 2).asString };
...
//		cs.learn = mButtonLearn.value == iLearn;
		...
		mNumberBoxClusters.value_(cs.nClusters);
		this.changed(this, \dialogSettings, cs.nClusters);  // forces a new palette
	}
	...
	updateData { | data |
		...
		if (mStarted, {
			...
			mNumberBoxHarmonics.value = cs.nHarmonics.asInteger;
	...
	drawResynthEGG { arg uView, clustNr, overlays;
		...
		var nSteps = 80;  // Could take this from VRPMovingEGG
		...
		if (mClusterCounts.notNil && mClusterCentroids.notNil, {
			...
			amps = [1.0] ++ (nDeltas collect: { |i| (mCC[i]*10).dbamp } );
			...
			shape = FloatArray.fill(nSteps+1, { arg ix;
				var phi = ix * phiStep;
				var acc = 0.0;
				// Resynthesize the EGG
				nDeltas.do { arg j; acc = acc + (amps[j] * cos((phi*(j+1)) + phases[j])) };
				acc
				// Or, resynthesize the dEGG (albeit with strong ripple)
				// nDeltas.do { arg j; acc = acc + (amps[j] * (j+1) * sin((phi*(j+1)) + phases[j])) };
				// acc.neg
			} ).normalize;
