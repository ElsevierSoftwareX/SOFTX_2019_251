...
VRPViewSampEn {
	...
	*new { | view |
	...
	init { | view |
		...
		mScopeViewer.view.background_(Color.black);

		///// Implement mouse-drag control of the plot duration  ///////////
		mbMouseDown = false;
		mbStretchOK = true;

		mScopeViewer.view.mouseDownAction = { arg view, x, y, modifiers, buttonNumber, clickCount;
			if (mbStretchOK, {
		...
		mScopeViewer.view.mouseMoveAction = { arg view, x, y, modifiers;
			if (mbMouseDown, {
		...
		mScopeViewer.view.mouseUpAction = { arg view, x, y, modifiers, buttonNumber, clickCount;
			if (mbStretchOK, {
				mbMouseDown = false;
				mXscale = 1.0;
				mXstart = mXnow;
		...
		[
			...
			mCheckBoxDrawCrest,
			mCheckBoxDrawSampEn
		] do: { |c, i|	c.palette = c.palette.base_(mCurveColors[i]) };
	...
	initMenu {
		...
		var general_font = Font(\Arial, 12, italic: true);
		...
		mCheckBoxDrawSampEn = CheckBox(mView, Rect(), "Sample Entropy →")
		.font_(static_font)
		.value_(false)
...
//		var duration = data.settings.scope.duration;
		...
		if (dsg.guiChanged, {
			...
			mScopeViewer.view.background_(dsg.getThemeColor(\backGraph));
