	...
	*new { | parentMenu |
	...
	init { | parentMenu |
		...
		mView.layout = VLayout.new(
			...
			[nil, s:20],
			HLayout([nil, s:20], [mCheckBoxSaveSettingsOnExit, s: 1, a: \left], [mButtonCancel, a: \right], [mButtonOK, a: \right])
		);
	...
	saveSettings {
		// Persistent settings implemented in VRPMain: window: OnClose
	}

	loadSettings {
		// Persistent settings implemented in VRPModel.loadSettings
	}
}