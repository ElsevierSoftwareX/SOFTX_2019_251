...
/**		// Init the start/prev for sampen arrows - DEPRECATED
...
**/
		...
		.addHandler(\vrp, \custom, {
			...
			data.dataAsFrames do: { | frame |
				...
				#freq, amp, clarity, crest, entropy, dEGGmax, qContact, iContact, clusterNumber = frame[ cycle_tool.indices(\VRPData) ];
				...
				value {
					...
					var meanDEGG = vrpd.dEGGmax.at(idx_spl, idx_midi) ? 0 ; // nil = 0
					var meanQc = vrpd.qContact.at(idx_spl, idx_midi) ? 0 ; // nil = 0
					var meanIc = vrpd.iContact.at(idx_spl, idx_midi) ? 0 ; // nil = 0
					var nm = density*meanCrest + crest;
					var nd = density*meanDEGG + dEGGmax;
					var nq = density*meanQc + qContact;
					var iq = density*meanIc + iContact;
					var invDensity, eTmp;
					density = density + 1;
					invDensity = density.reciprocal;
					meanCrest = nm * invDensity;
					meanDEGG = nd * invDensity;
					meanQc = nq * invDensity;
					meanIc = iq * invDensity;

					// Set the mean crest factor, dEGG, qContact and density
					vrpd.crest.put(idx_spl, idx_midi, meanCrest);
					vrpd.dEGGmax.put(idx_spl, idx_midi, meanDEGG);
					vrpd.qContact.put(idx_spl, idx_midi, meanQc);
					vrpd.iContact.put(idx_spl, idx_midi, meanIc);
					...
					percent = best_count * 100 * invDensity;
					vrpd.mostCommonCluster.put(idx_spl, idx_midi, [best_cluster_idx, percent]);

					// Update the entropy matrix
					eTmp = vrpd.entropy.at(idx_spl, idx_midi);
					if (eTmp.isNil or: {entropy > eTmp}, {
